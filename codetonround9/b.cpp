...
void Vedant(){
    string s;
    cin >> s;
    // substrings of aba -> a b a ab ba aba (n*(n+1))/2 for it to be even n must be odd
    // substring of abcd -> a b c d ab bc cd abc bcd abcd
    // so just return substring of size 3
    int n = sz(s);
    for (int i = 0; i + 1 < n; i++) {
        if (s[i] == s[i + 1]) {
        cout << s.substr(i, 2) << '\n';
