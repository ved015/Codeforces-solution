// #include<bits/stdc++.h>
#include<iostream>
#include<vector>
#include<algorithm>
#include<map>
#include<set>
#include<string>
using namespace std;

typedef long long ll;
typedef pair<int,int> pii;
typedef vector<int> vi;
typedef vector<string> vs;
typedef vector<long long> vll;
typedef vector<vector<ll> > vvll;

#define fastio() ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
#define pb push_back
#define mp make_pair
#define sz(x) ((int)(x).size())
#define all(x) x.begin(),x.end()

ll max(ll a, ll b, ll c){ return max(a, max(b, c));}
ll min(ll a, ll b, ll c){ return min(a, min(b, c));}
ll gcd(ll a, ll b) {if (b > a) {return gcd(b, a);} if (b == 0) {return a;} return gcd(b, a % b);}
ll fact(ll a) { return (a <= 1) ? 1 : a * fact(a - 1); }

vll factors(int n) { vll ans; int l = sqrt(n); for (int i = 1; i <= l; i++) { if (n % i == 0){ if (n/i == i) ans.pb(i); else {ans.pb(i); ans.pb(n/i);}}} return ans;}
string DecimalToBinary(ll num){string str; while(num){if(num & 1) str.pb('1');else str.pb('0');num>>=1;}return str;}
ll BinaryToDecimal(string num){ll ans = 0; ll base = 1; for (int i = sz(num) - 1; i >= 0; i--) {if (num[i] == '1') ans += base; base<<=1;}return ans;}

void Vedant(){
    int n;
    cin >> n;
    vector<pii> a(n),b;
    for(int i = 0; i < n; i++){
        int l,r;
        cin >> l >> r;
        a[i] = mp(l,r);
    }
    vi nums;
    map<int,int> pos;
    for(int i = 0; i < n; i++){
        if(a[i].first == a[i].second) pos[a[i].first]++;
    }
    for(auto it : pos) nums.pb(it.first);
    sort(all(nums));
    if(sz(nums) == 0){
        string s;
        for(int i = 0; i < n; i++) s += '1';
        cout << s << endl;
        return;
    }
    int mn = nums[0],mx = nums[0];
    for(int i = 0; i < sz(nums) - 1; i++){
        if(nums[i] + 1 == nums[i+1]) mx = nums[i+1];
        else{
            b.pb(mp(mn,mx));
            mn = nums[i+1];
            mx = nums[i+1];
        }
    }
    b.pb(mp(mn,mx));
    // for(auto it : b){
    //     cout << it.first << " " << it.second << endl;
    // }

    string s;
    for (int i = 0; i < n; i++) {
        int l = a[i].first,r = a[i].second;
        if(l == r){
            if(pos[l] > 1) s += '0';
            else s += '1';
            continue;
        }
        else{
            int left = 0,right = sz(b) - 1;
            bool flag = false;
            while(left <= right){
                int mid = left + (right - left)/2;
                if(b[mid].first <= l && b[mid].second >= r){
                    flag = true;
                    break;
                }
                else if(b[mid].first >= r) right = mid - 1;
                else left = mid + 1;
            }
            if(flag) s+= '0';
            else s += '1';
        }
    }
    // cout << "test ended" << endl;
    cout << s << endl;
}


int main(){
    fastio();
    int t;
    cin >> t;
    while(t--){
       Vedant();
    }
    return 0;
}