...
void Vedant(){
    int n;
    cin >> n;
    vi monocarp(n),stereocarp(n);
    for(int i = 0; i < n; i++) cin >> monocarp[i];
    for(int i = 0; i < n; i++) cin >> stereocarp[i];

    int ans = 0;
    for(int i = 0; i < n; i++){
        if(i + 1 < n && monocarp[i] > stereocarp[i + 1]) ans += monocarp[i] - stereocarp[i + 1];
    }
    cout << ans + monocarp[n - 1] << endl;
}
