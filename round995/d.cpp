...
void Vedant(){
    ...
    ll max_removal = sum - x;
    ll min_removal = sum - y;

    sort(a.begin(), a.end());
    ll count = 0;

    for(int i = 0; i < n - 1; i++) {
        ll target_min = min_removal - a[i];
        ll target_max = max_removal - a[i];

        auto low = lower_bound(a.begin() + i + 1, a.end(), target_min);
        auto high = upper_bound(a.begin() + i + 1, a.end(), target_max);
